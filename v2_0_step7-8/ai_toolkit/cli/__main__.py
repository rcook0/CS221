...
def _cmd_run_tram(args: argparse.Namespace) -> int:
    ...
    if args.trace_out:
        ...
        write_search_trace_jsonl(
            res.trace,
            args.trace_out,
            max_edges=args.trace_max_edges,
            result=res,
            context={
                "domain": "tram",
                "algo": algo,
                "N": int(problem.N),
                "walk_cost": float(costs.walk),
                "tram_cost": float(costs.tram),
            },
        )
...
def _cmd_run_tram_mdp(args: argparse.Namespace) -> int:
    ...
    algo = args.algo.lower()
    if algo in ("value", "value_iteration", "vi"):
        res = value_iteration(
            mdp,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    bench_tram.add_argument("--seed", type=int, default=0, help="seed recorded into bench rows")
