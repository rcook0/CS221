...
## Public API (Stable in v2.0)

If you are consuming **ai_toolkit** as an internal library, prefer importing from the `ai_toolkit.core.*` modules below.
These symbols are treated as the **stable surface** in v2.0.x (no breaking changes without a major version bump).

### Core protocols
- `from ai_toolkit.core.protocols import SearchProblem, MDP, ZeroSumGame`

### Search
- `from ai_toolkit.core.search.algorithms import bfs, ucs, astar`
- (Optional helpers) `from ai_toolkit.core.search.heuristics import ...` *(heuristics are stable only if explicitly documented there)*

### MDP
- `from ai_toolkit.core.mdp.algorithms import value_iteration, policy_iteration`

### Games
- `from ai_toolkit.core.games.algorithms import minimax, alphabeta`

### Results and schemas
- `from ai_toolkit.core.results import SearchResult, MDPResult, GameResult`
- `from ai_toolkit.core.traces import write_search_trace_jsonl, TRACE_SCHEMA_VERSION`
- `from ai_toolkit.cli.bench import BENCH_SCHEMA_VERSION` *(schema constant; CLI module is stable at the flag level)*

### Determinism utilities
- `from ai_toolkit.core.rng import make_rng, seed_numpy`

### Stability notes
- `ai_toolkit.domains.*`: **stable-ish** reference implementations (may grow; existing behavior should not break within v2.0.x).
- `ai_toolkit.viz.*`: supported, but viewer internals may change; treat `viz.trace_html` as the supported entrypoint.
- `ai_toolkit.cli.*`: CLI flags are considered stable within v2.0.x.
- `ai_toolkit.experimental.*`: **unstable** by design; anything here may change or be removed without notice.
